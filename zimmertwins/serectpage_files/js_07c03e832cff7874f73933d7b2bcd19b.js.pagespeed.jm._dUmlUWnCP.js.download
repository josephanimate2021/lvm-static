...
// replace the jQuery.clean function declared in the packed jquery.js file,
// with a non-packed one to work around ActiveX control activation in IE6
jQuery.clean = function( elems, context ) {
	var ret = [];
	context = context || document;
...
$(function(){
	...
	// embed flash using livequery so that content returned 
	// ajax requests can return content containing flash
  $('.flash:not(.flash-replaced)').livequery(function(){
    
		// options for jquery.flash
    var options = {
      flashvars: {}
    };
    
		// find flashvars
    $('.flashvars input:hidden', this).each(function(){
      var name = $(this).attr('name');
      var value = $(this).attr('value');
			if(value && value.length) {
      	options.flashvars[name] = value;
      }
    });

		// remove flashvars
    $('.flashvars', this).remove();
    
		// find attributes/params
    $('input:hidden', this).each(function(){
      ...
      if(value && value.length) {
		...
		// remove attributes/params
    $('input:hidden', this).remove();
...
/**
 * Override the l10n client's toggle function, which has some nasty inline css.
 */
if(Drupal.l10nClient) {
    Drupal.l10nClient.toggle = function(state) {
      switch(state) {
        case 1:
          $('#l10n-client').removeClass('hidden');
          $('#l10n-client .labels .toggle').text(Drupal.t('Close'));
          $.cookie('Drupal_l10n_client', '1', {expires: 7, path: '/'});
        break;
        case 0:
          $('#l10n-client').addClass('hidden');
...
$(function(){
	$('#page-moviepick-contest-vote .contest-entry:not(.contest-entry-processed)').livequery(function(){
		...
		$action.bind('click', function(event){
			$action.unbind('click', arguments.callee);
		...
		$this.addClass('.contest-entry-processed');
		
	});
});

Drupal.theme.setMessage = function(html, type) {
	
	var $list = $('.messages');
	if(!$list.length) {
		$list = $('<ul class="messages"></ul>');
		$list.insertAfter('h1');
	}
	$list.empty();
...
Drupal.theme.onModerateContentComplete = function(response, target) {
	$(target).slideUp('slow', function(){
		$(target).remove();
		Drupal.theme.setMessage(response.data, 'status');
		$('.tabs .active a').each(function(){
			this.innerHTML = this.innerHTML.replace(/\d+/, function($0){
...
Drupal.theme.onModerateContentError = function(response, target) {
	console.log(response);
	Drupal.theme.setMessage(response.data, 'error');
};;
